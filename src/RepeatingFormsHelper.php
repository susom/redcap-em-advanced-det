...
class RepeatingFormsHelper
{
    ...
    public function __construct($pid, $instrument_name)
    {
        ...
        if (!$this->is_longitudinal) {
            $this->event_id = $this->Proj->firstEventId();// array_keys($this->Proj->eventInfo)[0];
        }

        // See if this project has repeating forms/events
        $all_events = $this->Proj->getRepeatingFormsEvents();

        \Plugin::log($all_events);
